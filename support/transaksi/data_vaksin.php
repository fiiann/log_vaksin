...
<?php
	...
	?>
		...
		<tr class='data-header' bgcolor="#4F4E33">
			...
			<td align="center">Jml Stok </td>
			<td align="center">Jml Order</td>			
	...
	<?php
		...
		while ($out=mysql_fetch_array($qrydetil))
		{
			...
			if ($hasil<=0)
			{
				...
				$idpus=$_POST['kode_puskesmas'];
				
				//cek data obat untuk menghindari double data pada add order obat
				$sqlcros = "select 
							 t.kode_vaksin
							,t.jml_order
							,t.satuan
							...
							,b.kode_vaksin
							,tx.idorder
							from trxprov_detil t
							INNER JOIN master_vaksin b
							 on t.kode_vaksin=b.kode_vaksin
							INNER JOIN trxprov tx		
							where t.kode_vaksin='".$kodevaksin."' AND t.is_order=0";
							
				$qrycros = mysql_query($sqlcros);			
				while ($out=mysql_fetch_array($qrycros))
				{
					$qty_order=$out['qty_order'];					 
				}				
				
				//cek vaksin 
				$chk	= "select kode_vaksin from trxprov_detil where kode_vaksin='".$kodevaksin."'";
				$qchk	= mysql_query($chk);
				$cek	= mysql_num_rows($qchk);
				
				if ($cek!=0)
				{?>
					<tr class='data-row' id='select' bgcolor="#ababab">
				<?php 
					}else if ($hasil<=0){?>
			...
			<?php 
					...
					<td width="10%" align="center" class="cekstok" id="<?php echo $kodevaksin;?>">											
						<input type="text" name="jmlorder[]" id="jmlorder_<?php echo $kodevaksin;?>" size="5" value="<?php echo $jmlstok;?>" style='text-align:right'>				
			...
			<?php 
				...
				if ($cek!=0)
				{?>
					<td width="10%" align="center" class="terpilih"><input type="checkbox" name="checked[]" class="checked" id="checked" checked disabled></td>			
			<?php
				}else if ($hasil<=0){?>
