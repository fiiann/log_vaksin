...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#boxreport").dialog({
					...
					width:1050,
					height:600,
					title:'View Report Pemakaian Vaksin Puskesmas', 
					top:20, 
					modal:true, 
					buttons:{
						'Close' :function(){ 
		 ...
		 $("#jenisvaksin").change(function(){
			var jenis = $("#jenisvaksin").val();
			$.ajax({
					 type :"POST",
					 url  :"movetableview.php",
					 cache:false,
					 data :"jenis="+jenis,
					 success : function(html)
					 {
		 ...
		 $("#tampilkan").click(function(){
				var kodepkm  = $("#kode_pkm").val();
				var blnpakai = $("#blpakai").val();
				var nmbulan	 = $("#blnpakai").val();
				...
				if((kodepkm!='') && (blnpakai!=''))
				{
					...
					$.ajax({
						 ...
						 url :"viewreport_pkm.php",
						 cache:false,
						 data :"kode_pkm="+kodepkm+"&blnpakai="+blnpakai+"&namabln="+nmbulan,
						 success : function(html){
					...
					alert('Kolom Puskesmas dan Kolom Periode masih kosong');
				}
		 });
...
<?php
	...
	//get data instansi
	$inst = "SELECT kode_instansi, nama from instansi ORDER by kode_instansi";
	$qins = mysqli_query($koneksi,$inst) or die (mysql_error());
?>	
	<div id="listview_container">

		<div class="form_description">
			<h3 style="font-family:Tahoma">&nbsp;&nbsp;Laporan Monitoring Vaksin Imunisasi</h3>		 
		</div>		
		&nbsp;&nbsp;<span id="nm_bulan" style="font-family:Tahoma;font-size:13px"> <b>Bulan :</b></span> <select id="bulan">
		<?php
