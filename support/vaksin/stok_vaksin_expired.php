...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#simpan_exp").click(function(){
			...
			var aksi  = "insert";
			if ((jenis.length>0) && (s_exp.length>0)) 
			{
				
				$.ajax({
						type 	: "POST",
						url	 	: "simpan_stok_expired.php",
						data 	: "aksi="+insert+"&id_jenis="+jenis+"&bulan="+bulan+"&tahun="+tahun+"&stok_awal="+s_awl+'&stok_expired='+s_exp+'&satuan='+satuan,
...
<body id="main_body" >
	...
	<div id="form_container">
		...
		<h1><a>Input Stok Vaksin Expired</a></h1>
		...
		<div>
			<input id="stok_tersedia" name="stok_tersedia" class="element text small" type="text" maxlength="255"/> 
		</div> 
		</li>
		<li id="li_6" >
		<label class="description" for="element_6">Jumlah Stok *</label>
		<div>
			<input id="stok_expired" name="stok_expired" class="element text small" type="text" maxlength="255"/> 
		</div> 
		</li>	
        <li id="li_6" >
		<label class="description" for="element_6">Satuan *</label>
		<div>
			<input id="satuan" name="satuan" class="element text small" type="text" maxlength="255" value=""/> 
		</div> 
		</li>
		<li class="buttons">
			    <input type="hidden" name="form_id" value="898777" /> 
				<input id="simpan_exp" class="button_text" type="button" name="simpanstok" value="Simpan Stok" />
		</li>
		<div id="confirm"></div>
			</ul>
		</form>	
